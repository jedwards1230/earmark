...
# If a gpu-arbiter binary resolves (PATH, or --arbiter-wait-cmd/$ARBITER_WAIT_CMD),
# the yield delegates to its `wait`/`status -q` subcommands instead of the
# built-in HTTP poll loop — optional, never a hard dependency (CONTRACT §1.4).
```
...
`MCP_HTTP_ADDR`, `STALE_JOB_TIMEOUT`, `SCAN_INTERVAL`, `CHUNK_SIZE`,
...
| `internal/eval` | Read-only LLM-as-judge (CONTRACT §2.15, `earmark eval`): READS transcript chunks, INSERTs `transcript_findings` as **proposed patches**. NEVER edits transcripts — applying is `internal/patch`'s job. Replies are pinned to a JSON schema via `response_format` (verified supported by Ollama's OpenAI-compatible endpoint); a thinking-only reply fails loudly with `ErrThinkingOnlyResponse` rather than being read as "no findings". Chat endpoint resolved via `AI_ROLES["eval"]` (registry, CONTRACT §2.14) falling back to `EVAL_CHAT_*` env vars. |
| `internal/patch` | Human-gated **replayable correction overlay** for eval findings (CONTRACT §2.17). `transcript_chunks` is a disposable projection — the worker rebuilds it from the immutable transcript source on every embed — so corrections are never written into chunk text; they live in `transcript_findings` and are **replayed** onto the regenerated text: `ApplyCorrections` (pure, strict) / `Replay` (lenient, quarantines unusable patches as `stale` with a reason). Resolves all spans against the pristine text and splices in **descending** offset order, which is what makes a rebuild byte-identical. Also owns the `proposed→accepted→applied→reverted` state machine, rune-indexed anchor resolution (`Locate` refuses rather than guessing when a span is ambiguous), and chunk-hash verification so a correction never edits a revision the judge did not review. Separate from `internal/eval` **on purpose**: it is what keeps §2.15's read-only guarantee mechanically true. |
| `internal/batch` | `earmark batch` coordinator core (CONTRACT §1.4): drives `runner_control.phase` + `run_limit` to run the pipeline in transcribe→analyze batches so the ASR model and eval judge time-share one GPU. Reads gpu-arbiter `/status` (read-only) to yield to games — either via a built-in HTTP poll loop, or, when a gpu-arbiter binary resolves (PATH auto-detect or `--arbiter-wait-cmd`/`ARBITER_WAIT_CMD`), delegated to the gpu-arbiter CLI's `wait`/`status -q` subcommands (optional; never a hard dependency). Dependency-injected (`PhaseStore`, `Arbiter`, optional `Waiter`) so phase transitions are unit-testable; always restores idle on exit; DB-driven/resumable. |

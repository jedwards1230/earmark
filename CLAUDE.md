...
| `internal/patch` | Human-gated apply path for eval findings (CONTRACT §2.17). Owns the `proposed→accepted→applied→reverted` state machine, rune-indexed anchor resolution (`Locate` refuses rather than guessing when a span is ambiguous), and chunk-hash verification so a patch never edits a revision the judge did not review. Separate from `internal/eval` **on purpose**: it is what keeps §2.15's read-only guarantee mechanically true. |

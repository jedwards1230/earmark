...
The MCP HTTP transport also serves an htmx dashboard (LAN-only). **Home (`/`) is the Library page.** The **Pipeline ops page** (`/pipeline`) hosts the auto-refreshing `/status/data` fragment (every 3 s) — counts, pipeline state, a **read-only phase badge**, and token-gated **pause + run-budget** controls — with the **Failed jobs view folded in** (the standalone `/failed` page is removed; `/failed/data` is kept). A compact read-only phase + paused badge also appears in the topbar on every page, linking to `/pipeline`. See CONTRACT §2.12.1 for the full page-route inventory.

The dashboard **reads** `runner_control.phase` but never writes it — the `earmark batch` coordinator owns phase transitions (CONTRACT §1.4). Only `paused`/`run_limit` are dashboard-writable, via the htmx actions guarded by the `HX-Request` header; the pause + run-budget actions (`/actions/pause`, `/actions/resume`, `/actions/run?n=N`, `/actions/run-clear`) additionally fail closed (render disabled) when `CONTROL_API_TOKEN` is unset. Requeue buttons (`/actions/requeue?id=…`, `/actions/retry-failed`) stay htmx-only.

The **Library** list (`/library`, `/library/data`) supports a **sort** control — `recent` (default, transcribed-first), `title`, `progress`, `findings` — and a **⚑ has-findings** filter (`?findings=1`), both applied all-in-Go over the full filtered set so they span the whole library, not just one page.

The **`/findings`** page (read-only eval layer, CONTRACT §2.15) and the per-book **`/book`** detail section both render the **individual finding rows** — confidence, issue type, `original → suggested correction`, and where — sorted by confidence DESC, in addition to the per-book roll-up. Each finding's **Where** cell deep-links to the transcript at the suspected point (`/track?id=…&t=<startSec>`; the reader preloads through that segment, highlights it, and scrolls to it), falling back to a plain label when the finding has no job id. The Book page also exposes the scoped clear (`POST /actions/findings-clear?dir=…`, token-gated) which re-renders the Book fragment, and a `⚑ N` per-track flag column linking into the book's findings section. All read-only/advisory — no transcript is ever modified.
